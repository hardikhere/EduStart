...
import { ValidateEmail, validatePhone, validatePinCode } from '../../../utils/common';
...
const useFormValidation = () => {
    ...
    useEffect(() => {
        if (!ValidateEmail(schoolRegister.email))
            setemailErr(true)
        else
            setemailErr(false)
        ...
        if (!schoolRegister.yearOfEstablishment ||
            schoolRegister.yearOfEstablishment.length !== 4)
            setyoeErr(true)
        else setyoeErr(false);

        if (!schoolRegister.phoneNumber ||
            !validatePhone(schoolRegister.phoneNumber))
            setphoneErr(true)
        else setphoneErr(false);

        if (!schoolRegister.pinCode ||
            !validatePinCode(schoolRegister.pinCode))
            setpinCodeErr(true);
