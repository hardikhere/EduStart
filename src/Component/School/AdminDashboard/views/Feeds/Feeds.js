...
const Feeds = () => {
    ...
    const postFeed = () => {
        ...
        axiosInstance.post(APIS._addFeed, {
            ...UserResponse,
            sid: schoolAdminDetails?.schoolDetails?.schoolId
        })
            .then(res => {
    ...
    const getFeeds = () => {
        ...
        axiosInstance.get(APIS._getFeeds(
            schoolAdminDetails.schoolDetails.schoolId
        ))
            .then(res => {
    ...
    useEffect(() => {
        if (!!schoolAdminDetails?.schoolDetails?.schoolId)
            getFeeds();
    ...
    const AddNewModal = () => {
        return (<div>
            ...
            <Modal.Body style={{ maxHeight: "22rem", overflowY: "scroll" }}>
                <Form>
                    {hasErrors && <h6 className="text-danger">
                    ...
                    <Form.Group>
                        ...
                        <Form.Control type="text" value={UserResponse.title}
                            onChange={handleChange("title")} ></Form.Control>
                    </Form.Group>
                    ...
                    <Form.Group>
                        ...
                        <Form.Control onChange={handleChange("content")}
    ...
    const updateFeedController = () => {
        ...
        setSubmitLoading(true);
        axiosInstance.put(APIS._updateFeed, {
            ...UserResponse,
            sid: schoolAdminDetails?.schoolDetails?.schoolId
        })
            .then(res => {
    ...
    const loadingCards = () => {
        if (FeedsLoading) {
            return <SkeletonTheme color="#E5E5E5" highlightColor="#F3F3F3">
                <div className="d-flex flex-wrap justify-content-center"
                    style={{ width: "100%" }}>
                    <Skeleton height={"20rem"} width={"18rem"}
                        style={{ margin: "1rem" }}
                    />
                    <Skeleton height={"20rem"} width={"18rem"}
                        style={{ margin: "1rem" }}
    ...
    const deletePost = (id) => {
        axiosInstance.delete(APIS._deleteFeed(id))
            .then(res => {
                console.log(res.data);
    ...
    return (
        <div className="p-1">
            <Modal style={{ marginTop: "4.3%" }} show={showNewFeedModal} onHide={handleClose}>
                {AddNewModal()}
            ...
            <div className="d-flex flex-wrap justify-content-center">
                {loadingCards()}
                <ConfirmDeleteModal show={showDeleteModal} name="feed"
                    handleClose={() => setshowDeleteModal(false)}
                    onDelete={() => deletePost(toDeleteId)}
                />
                {
                    Feeds.map(post => {
                        return <div className="feedCard" key={`das${post}`}>
                            <div className="feedCard-option">
                                <div className="btn btn-primary m-1" onClick={() => handleEditBtn(post)}>
                                    <FontAwesomeIcon icon={faEdit} />
                                </div>
                                <div className="btn btn-danger m-1" onClick={() => {
                            ...
                            <div className="feedCard-img" style={{
                                backgroundSize: "cover"
                            }}>

                                <img src={post.imageUrls[0]} style={{ height: "100%" }}
                                    alt="" srcset="" />
                            </div>
                            <div className="p-1">
                                <h5>{post.title}</h5>
