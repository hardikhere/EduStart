...
const ShortListIcon = (props) => {
    ...
    useEffect(() => {
        const shortlist = JSON.parse(window.localStorage.getItem("shortlist")) || [];
    ...
    return (
        <React.Fragment>
            {
                props.isProfile ? <Button onClick={handleShortList}>
                    {!isShortlisted ? <i className="pi pi-bookmark mr-1"></i> :
                        <FontAwesomeIcon icon={faBookmark} className="mr-1" />
                    }
                    {
                        isShortlisted ? "Shortlisted" : "Shortlist"
                    }
                </Button> : <div className="mark-container" onClick={handleShortList}>
                    {!isShortlisted ? <i className="pi pi-bookmark filled"></i> :
