...
const SearchFilter = () => {
    ...
    const handleFilterInclude = (name, el) => {
        ...
        if (window.location.search.includes(el)) {
            ...
            let newSearchString = parsedqs[name]?.substr(0, index) +
    ...
    return (
        <div className="filterbox">
            <h5 className="filterbox-heading">
                Customize Search:
            </h5>
                ...
                <div className="d-flex flex-wrap">
                    {
                        AvailableFilters.classification.map(el => {
                            return <div key={`${el}`} onClick={() => handleFilterInclude("classification", el)}
                                className={`filterbox-btn
                            ${window.location.search.includes(el) ? "btn-selected" : ""}`}>{el}</div>
                        })
                    }
                </div>
            </div>
                ...
                <div className="d-flex flex-wrap">

                    {
                        AvailableFilters.board.map(el => {
                            return <div onClick={() => handleFilterInclude("board", el)} className={`filterbox-btn
                            ${window.location.search.includes(el) ? "btn-selected" : ""}`}>{el}</div>
                        })
                    }
                </div>
                ...
                <div className="d-flex flex-wrap">
                    {
                        AvailableFilters.fees.map(el => {
                            return <div key={`${el}`} onClick={() => handleFeeFilter(el)}
                                className={`filterbox-btn
                            ${parseInt(parsedqs.feeTo) === el.feeTo && "btn-selected"
                                    }`}>{el.tag}</div>
                        })
                    }
                </div>
                ...
                <div className="d-flex flex-wrap">
                    {
                        AvailableFilters.schoolType.map(el => {
                            return <div key={`${el}`} onClick={() => handleFilterInclude("schoolType", el)}
                                className={`filterbox-btn
                            ${window.location.search.includes(el) ? "btn-selected" : ""}`}>{el}</div>
                        })
                    }
